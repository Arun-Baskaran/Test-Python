    ...
    def run_command(self, cmd, sudoenabled=False):
        ...
        if len(output) == 0:
            ...
            with open(file1,  'a') as wf:
                wf.write("ClamAV not installed in the server -  {}".format(hostname))
        else:
            print("ClamAV is installed in the server {}".format(hostname))
            with open(file2,  'a') as wf:
                wf.write("ClamAV installed in the server -  {}".format(hostname))
        session.close()
        return output
...
def main():
    ...
    with open(Server_List, 'r') as line:
        for hostname in line:
            print ("Connecting to the server - {}".format(hostname))
            global hostname
            ssh = PySSH()
            ssh.connect(hostname, passwd, port)
            output = ssh.run_command('yum list installed clamAv', True)
            print("Closing Connection")
            ssh.disconnect()
            print("Disconnected from the server - {0}".format(hostname))
            continue
            print("")
            return
